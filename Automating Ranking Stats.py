...
powers[0] = np.array([50, 1, 5, 1], dtype = float)
...
#this will keep track of how good at picking the host galaxy each combination of powers is
Ranking_place = np.zeros(shape = (N, Num))



#setting up a dataframe to store the Best ranks
column_names = ["Rank Type", "Distinction", "Cutoff Value", "a", "b", "c", "d"]
Iterative_rank = pd.DataFrame(index = range(Num+1), columns = column_names)

#initial conditions
inital = ["N/A", 0.7*N, 0, 1, 1, 1, 1]
...
for j in range(Num):
    ...
    if Trigger == False:
        ...
        Temp = [best_type, distinction_level, AVG, *potential_powers]
