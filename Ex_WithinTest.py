...
def rank22(theta, sigma, d_lum, luminosity, luminosity_probability): ## Optimise 2
    return np.exp(-(theta**2/(2 * (sigma)**2)))**(sigma**4) * (1/d_lum**8 * luminosity)[:, 0] * luminosity_probability**2
...
def find_nearest(array, value): #Kind of a hash and not exactly interpolation, but for this point, should be okay
    ...
    return idx    
#################################################################
def reduction(RA_dec, Dec_dec, df_master, error_radius):  ##Reduces the df_master by considering angular distance
    
    #host = df_master.iloc[current_i]
    RA_dec = ra_prime[i]#host[["RA"]].values.tolist()[0]
    Dec_dec = dec_prime[i]#host[["dec"]].values.tolist()[0]    
...
test_ra = df_master[["RA"]].values.tolist()
test_dec = df_master[["dec"]].values.tolist()
...
"""
print(len([i for i, val in enumerate(placement21) if val == [1]]))
print(len([i for i, val in enumerate(placement22) if val == [1]]))
print(len([i for i, val in enumerate(placement23) if val == [1]]))
...
for i in range(N):
    ...
    testr = test_ra[current_i][0]  #np.array(test_ra.iloc[[current_i]].values.tolist())[0][0]
    testd = test_dec[current_i][0] #np.array(test_dec.iloc[[current_i]].values.tolist())[0][0]
...
####################################################################################################    
    ...
    ra_prime[i] = testr + (ra_prime[i] - 50)
    dec_prime[i] = testd + (dec_prime[i] - 10)
...
###################################################################################################
    ...
    im_check = [i for i, val in enumerate(df_master[["Identifier"]].values.tolist()) if val == 1]
    #galra = 
    
    q, t, df_sliced = reduction(ra_prime[i], dec_prime[i], df_master, error_radius) ## Reduces the catalog by RA and dec
    ...
    if df_sliced.shape[0] == 0:
        ...
        for k in range(df_sliced.shape[0]):
            angular_distaance[k] = Ang_Dist(ra[k], testr, dec[k], testd)
        ...
        angular_distance = Ang_Dist(ra, testr, dec, testd)
        """
        if len(id_check) != 0:
            break
        else:
        ...
        if len(mask_check20) == 0:
            ...
            #display(Markdown("The keplerian orbit appears to be happening at r ={0:.2f} km"  .format(float(kepler(M_kep, w))/1000)))
            print("Galaxy data: \nDistance is {0:.2f} Mpc\nLuminosity is {1:.3e}\nra and dec [{2:.2f}, {3:.2f}] compared to reported ra and dec [{4:.2f}, {5:.2f}] \nTrue luminosity {6:.3e} W" .format(dl[int(placement[i] - 1)][0], Luminosity[int(placement[i] - 1)][0], fin_ra[int(placement[i] - 1)][0], fin_dec[int(placement[i] - 1)][0], testr, testd, b[i]))
            ...
            #ras_dex[i, 0] = fin_ra[int(placement[i] - 1)]; ras_dex[i, 1] = fin_dec[int(placement[i] - 1)]; test_case[i, 0] = testr, test_case[i, 1] = testd
...
for i in range(23):
    print("Statistic " + str(i + 1) + ": " + str(np.mean(df_place_array[:, i])))
    
fig, axes = plt.subplots(2, 1, sharex = True)

ax = axes[0]
ax.plot(np.log10(df_distance), df_place_array[:, 21], "kx", label = "Statistic 22")
ax.set_ylabel("Percentage Performance")
ax.legend(loc = "upper left")
ax.grid()

ax = axes[1]
ax.plot(np.log10(df_distance), df_place_array[:, 22], "kx", label = "Statistic 23")
ax.set_ylabel("Percentage Performance")
ax.legend(loc = "upper left")
ax.grid()
plt.xlabel("Log$_{10}$(Distance) /Mpc")
plt.savefig("Optimisation comparison.png")
